...
class TestDuplicateProviderValidation:
    ...
    async def test_create_duplicate_provider_name(self, client, test_admin_api_key, test_db):
        ...
        response = client.post(
            "/api/portal/providers", headers=headers, json=provider_data
        )
    ...
    async def test_update_provider_duplicate_name(self, client, test_admin_api_key, test_db):
        ...
        response = client.post(
            "/api/portal/providers", headers=headers, json=provider_data1
        )
        assert response.status_code == 201
        ...
        response = client.post(
            "/api/portal/providers", headers=headers, json=provider_data2
        )
        assert response.status_code == 201
        ...
        response = client.put(
            f"/api/portal/providers/{provider2_id}", headers=headers, json=update_data
        )
        assert response.status_code == 400
        assert "already exists" in response.json()["detail"]
