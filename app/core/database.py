...
async def check_database_exists(session: AsyncSession) -> bool:
    ...
    try:
        ...
        if settings.database_type == "sqlite":
            result = await session.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='providers'"))
            providers_exists = result.fetchone() is not None
            result = await session.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='api_keys'"))
            api_keys_exists = result.fetchone() is not None
        else:
            # PostgreSQL
            result = await session.execute(text("SELECT table_name FROM information_schema.tables WHERE table_name = 'providers'"))
            providers_exists = result.fetchone() is not None
            result = await session.execute(text("SELECT table_name FROM information_schema.tables WHERE table_name = 'api_keys'"))
            api_keys_exists = result.fetchone() is not None
...
async def check_database_compatibility(session: AsyncSession) -> bool:
    ...
    try:
        ...
        required_provider_columns = {
            ...
            'model_list', 'big_model', 'small_model', 'medium_model',
            'is_active', 'priority', 'max_tokens', 'temperature_default',
            'headers', 'created_at', 'updated_at'
        }
        
        required_api_key_columns = {
            'id', 'key_name', 'api_key', 'description', 'is_active', 
...
async def init_db():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not db_info.get("tables_exist", False):
            ...
            print(f"Database exists with {db_info.get('providers_count', 0)} providers and {db_info.get('api_keys_count', 0)} API keys.")
            ...
            if is_compatible:
                ...
                print("Database schema is incompatible. Please backup data and recreate database.")
                raise RuntimeError("Incompatible database schema. Please backup your data and delete the database file to start fresh.")
