...
from app.schemas.anthropic import (
    AnthropicRequest, AnthropicResponse, CountTokensRequest, CountTokensResponse,
    ModelInfo, ModelListResponse, MessageBatchRequest, MessageBatch,
    MessageBatchResults, MessageBatchList, MessageBatchStatus,
    FileUpload, FileList, FilePurpose, TextContent, ToolUseContent,
    StopReason
)
...
async def list_models(
    ...
    try:
        providers = await ProviderService.get_all_providers(db)
...
async def get_model(
    ...
    try:
        ...
        for provider in providers:
            if model_id in [provider.small_model, provider.medium_model, provider.big_model]:
                return ModelInfo(
                    id=model_id,
                    created=int(datetime.now().timestamp()),
                    type="claude",
